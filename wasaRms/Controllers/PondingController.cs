...
namespace wasaRms.Controllers
{
    public class PondingController : Controller
    {
            ...
            using (SqlConnection conn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString))
            {
                try
                {
                    ...
                    foreach (DataRow drRes in dtRes.Rows)
                    {
                        ...
                        //query will get the list of data available against given resourceID (latest first)
                        string Dashdtquery = ";WITH cte AS ( SELECT DISTINCT r.resourceName AS Location, '3' AS Min_Level, s.parameterValue AS Current_Level, p.parameterUnit as pUnit, r.resourceNumber as rnum, DATEDIFF(minute, s.sheetInsertionDateTime, CONVERT(CHAR(24), CONVERT(DATETIME, GETDATE(), 103), 121)) as DeltaMinutes, s.sheetInsertionDateTime as tim,  ROW_NUMBER() OVER (PARTITION BY s.resourceID ORDER BY s.sheetInsertionDateTime DESC) AS rn FROM tblSheet s inner join tblResource r on s.resourceID = r.resourceID inner join tblParameter p on s.parameterID = p.parameterID inner join tblResourceType rt on r.resourceTypeID = rt.resourceTypeID where r.resourceID = " + resourceID + " and s.sheetInsertionDateTime > DATEADD(DAY, -30, GETDATE())) SELECT * FROM cte order by cast(rnum as INT) ASC, tim DESC ";
                        SqlCommand cmd = new SqlCommand(Dashdtquery, conn);
                        SqlDataAdapter sda = new SqlDataAdapter(Dashdtquery, conn);
                        Dashdt.Clear();
                        sda.Fill(Dashdt);
        ...
        public List<PondingSpellClass> getPondingTableList(DateTime fd, DateTime td, string res)
        {
            string printTitle = "";
            DataTable dtRes = new DataTable();
            DataTable Dashdt = new DataTable();
            ...
            using (SqlConnection conn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString))
            {
                try
                {
                    ...
                    if (Convert.ToInt32(Session["CompanyID"]) == 1)
                    {
                        ...
                        {
                            getResFromTemp = "select DISTINCT r.resourceID, r.resourceNumber, r.resourceLocationName from tblResource r inner join tblResourceType rt on r.resourceTypeID = rt.resourceTypeID inner join tblSheet s on r.resourceID = s.resourceID where rt.resourceTypeName = 'Ponding Points' and r.resourceName = '"+res+"' ";
                        } 
                    }
                    else
                    {
                        ...
                        {
                            getResFromTemp = "select DISTINCT r.resourceID, r.resourceNumber, r.resourceLocationName from tblResource r inner join tblResourceType rt on r.resourceTypeID = rt.resourceTypeID inner join tblSheet s on r.resourceID = s.resourceID where rt.resourceTypeName = 'Ponding Points'   and r.managedBy = " + Convert.ToInt32(Session["UserID"]) + " and r.resourceName = '" + res + "' ";
                        }
                    }
                    SqlDataAdapter sdaRes = new SqlDataAdapter(getResFromTemp, conn);
                    dtRes.Clear();
                    ...
                    foreach (DataRow drRes in dtRes.Rows)
                    {
                        ...
                        resourceLocation = drRes["resourceLocationName"].ToString();
                        //query will get the list of data available against given resourceID (latest first)
                        string Dashdtquery = ";WITH cte AS ( SELECT DISTINCT r.resourceName AS Location, '3' AS Min_Level, s.parameterValue AS Current_Level, p.parameterUnit as pUnit, r.resourceNumber as rnum, DATEDIFF(minute, s.sheetInsertionDateTime, CONVERT(CHAR(24), CONVERT(DATETIME, GETDATE(), 103), 121)) as DeltaMinutes, s.sheetInsertionDateTime as tim,  ROW_NUMBER() OVER (PARTITION BY s.resourceID ORDER BY s.sheetInsertionDateTime DESC) AS rn FROM tblSheet s inner join tblResource r on s.resourceID = r.resourceID inner join tblParameter p on s.parameterID = p.parameterID inner join tblResourceType rt on r.resourceTypeID = rt.resourceTypeID where r.resourceID = " + resourceID + "  and s.parameterValue >= 0  and s.sheetInsertionDateTime >= CONVERT(CHAR(24), CONVERT(DATETIME, '" + fd + "', 103), 121) and s.sheetInsertionDateTime < CONVERT(CHAR(24), CONVERT(DATETIME, '" + td + "', 103), 121)) SELECT * FROM cte order by cast(rnum as INT) ASC, tim DESC ";
                        SqlCommand cmd = new SqlCommand(Dashdtquery, conn);
                        SqlDataAdapter sda = new SqlDataAdapter(Dashdtquery, conn);
                        Dashdt.Clear();
                        sda.Fill(Dashdt);
                        if (Dashdt.Rows.Count > 0)
                        {
                            var tablList2 = getAllSpells(Dashdt, Convert.ToInt32(drRes["resourceNumber"]), fd);
                            //PondingSpellClass sd = getAllSpells(Dashdt, Convert.ToInt32(drRes["resourceNumber"]));
                            //tablList.Add(sd);
                            tablList.AddRange(tablList2);
                        }
                        else {
                            string Dashdtquery1 = "select '"+ resourceLocation + "' as Location, 0 as Min_Level, 0 as Current_Level, 'inch' as pUnit, 1 as rnum, 0 as DeltaMinutes, '"+fd+"' as tim, 1 as rn";
                            SqlCommand cmd1 = new SqlCommand(Dashdtquery1, conn);
                            SqlDataAdapter sda1 = new SqlDataAdapter(Dashdtquery1, conn);
                            Dashdt1.Clear();
                            sda1.Fill(Dashdt1);
                            var tablList2 = getAllSpells(Dashdt1, Convert.ToInt32(drRes["resourceNumber"]),fd);
            ...
            if (res == "All")
            {
                printTitle = "Report of All Ponding Locations between " + fd + " to " + td + "";
            }
            else
            {
                printTitle = "Report of Ponding at "+res+" between " + fd + " to " + td + "";
            }
                        ...
                        if (E == T && S == T)
                        {
                            ...
                            if (spelldata.SpellDataArray.Count > 1 && spelldata.SpellDataArray.Sum() > 0)
                            {
                                ...
                                if (spelldata.SpellMax == currentPonding)
                                {
                                    ...
                                    spelldata.spellFlowUp = Math.Round(spelldata.SpellMax / diffMinutes, 2);
                                    spelldata.spellFlowDown = 0;
                                }
                                else
                                {
            ...
            else
            {
                ...
                foreach (var item in spellDataList)
                {
                    ...
                    if (tableData.comment == "Continue...")
                    {
                        ...
                        tableData.spellFlowDown = Math.Round((DiffForSpellDown/TimeDifference), 2).ToString();
                        tableData.SpellEndTime = "-";
                    }
            ...
            using (SqlConnection conn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString))
            {
                try
                {
                    ...
                    foreach (DataRow drRes in dtRes.Rows)
                    {
                        ...
                        resourceLocation = drRes["resourceLocationName"].ToString();
                        //query will get the list of data available against given resourceID (latest first)
                        string Dashdtquery = ";WITH cte AS ( SELECT DISTINCT r.resourceName AS Location, '3' AS Min_Level, abs(s.parameterValue) AS Current_Level, p.parameterUnit as pUnit, r.minThr as minTh, r.maxThr as maxTh, r.resourceNumber as rnum, DATEDIFF(minute, s.sheetInsertionDateTime, CONVERT(CHAR(24), CONVERT(DATETIME, GETDATE(), 103), 121)) as DeltaMinutes, s.sheetInsertionDateTime as tim,  ROW_NUMBER() OVER (PARTITION BY s.resourceID ORDER BY s.sheetInsertionDateTime DESC) AS rn FROM tblSheet s inner join tblResource r on s.resourceID = r.resourceID inner join tblParameter p on s.parameterID = p.parameterID inner join tblResourceType rt on r.resourceTypeID = rt.resourceTypeID where r.resourceID = " + resourceID + " and s.parameterValue >=0  and s.sheetInsertionDateTime > CONVERT(date, getdate()) ) SELECT * FROM cte order by cast(rnum as INT) ASC, tim DESC ";
                        SqlCommand cmd = new SqlCommand(Dashdtquery, conn);
                        SqlDataAdapter sda = new SqlDataAdapter(Dashdtquery, conn);
                        Dashdt.Clear();
                        sda.Fill(Dashdt);
                        ...
                        {
                            string Dashdtquery1 = "select '" + resourceLocation + "' as Location, 0 as Min_Level, 0 as Current_Level, 'inch' as pUnit, 1 as rnum, 100 as DeltaMinutes, DATEADD(hour,0,GETDATE()) as tim, 1 as rn";
                            SqlCommand cmd1 = new SqlCommand(Dashdtquery1, conn);
                            SqlDataAdapter sda1 = new SqlDataAdapter(Dashdtquery1, conn);
                            Dashdt.Clear();
                            sda1.Fill(Dashdt);
        ...
        public ActionResult Dashboard()
        {
            ...
            if (Convert.ToInt32(Session["CompanyID"]) == 1)
            {
                query = "SELECT TOP 22 WITH TIES 'Ponding Points' as Type_, r.resourceName as category, r.maxThr as maxTh, 'Lahore' as townName, r.resourceName as townName2, round(t.parameterValue, 2) as ponding, 'ponding' as pName, 'inch' as unit, r.resourceLocationName, SUBSTRING(r.resourceGeoLocatin, 1, CASE CHARINDEX(',', r.resourceGeoLocatin) WHEN 0 THEN LEN(r.resourceGeoLocatin) ELSE CHARINDEX(',', r.resourceGeoLocatin) - 1 END) AS lat , SUBSTRING(r.resourceGeoLocatin, CASE CHARINDEX(',', r.resourceGeoLocatin) WHEN 0 THEN LEN(r.resourceGeoLocatin) + 1 ELSE CHARINDEX(',', r.resourceGeoLocatin) + 1 END, 1000) AS lng, t.sheetInsertionDateTime ,DATEDIFF(minute, t.sheetInsertionDateTime, CONVERT(CHAR(24), CONVERT(DATETIME, GETDATE(), 103), 121)) as DeltaMinutes FROM tblSheet t inner join tblResource r on t.resourceID = r.ResourceID inner join tblResourceType rt on r.resourceTypeID = rt.resourceTypeID where rt.resourceTypeID = 1 ORDER BY ROW_NUMBER() OVER(PARTITION BY r.resourceID ORDER BY t.sheetInsertionDateTime DESC)";
                //S1query = ";WITH cte AS ( SELECT rt.resourceTypeName as Type_, r.resourceName as category, 'Lahore' as townName, r.resourceName as townName2, s.parameterValue AS ponding, p.parameterName as pName, p.parameterUnit as unit, SUBSTRING(r.resourceGeoLocatin, 1, CASE CHARINDEX(',', r.resourceGeoLocatin) WHEN 0 THEN LEN(r.resourceGeoLocatin) ELSE CHARINDEX(',', r.resourceGeoLocatin) - 1 END) AS lat ,SUBSTRING(r.resourceGeoLocatin, CASE CHARINDEX(',', r.resourceGeoLocatin) WHEN 0 THEN LEN(r.resourceGeoLocatin) + 1 ELSE CHARINDEX(',', r.resourceGeoLocatin) + 1 END, 1000) AS lng, ROW_NUMBER() OVER (PARTITION BY s.resourceID ORDER BY s.sheetInsertionDateTime DESC) AS rn, s.sheetInsertionDateTime, DATEDIFF(minute, s.sheetInsertionDateTime, CONVERT(CHAR(24), CONVERT(DATETIME, GETDATE(), 103), 121)) as DeltaMinutes FROM tblSheet s inner join tblResource r on s.resourceID = r.resourceID inner join tblResourceType rt on r.resourceTypeID = rt.resourceTypeID inner join tblResourceTypeParameter rtp on rt.resourceTypeID = rtp.resourceTypeID inner join tblParameter p on rtp.parameterID = p.parameterID where rt.resourceTypeName = 'Ponding Points') SELECT COUNT(*) FROM cte WHERE rn = 1";
                S2query = "select count( DISTINCT r.resourceID )from tblSheet s ";
            ...
            {
                query = "SELECT top(select COUNT(resourceID) from tblResource where managedBy = " + Convert.ToInt32(Session["UserID"]) + ") WITH TIES 'Ponding Points' as Type_, r.resourceName as category, r.maxThr as maxTh, 'Lahore' as townName, r.resourceName as townName2, round(t.parameterValue, 2) as ponding, 'ponding' as pName, 'inch' as unit, r.resourceLocationName, SUBSTRING(r.resourceGeoLocatin, 1, CASE CHARINDEX(',', r.resourceGeoLocatin) WHEN 0 THEN LEN(r.resourceGeoLocatin) ELSE CHARINDEX(',', r.resourceGeoLocatin) - 1 END) AS lat , SUBSTRING(r.resourceGeoLocatin, CASE CHARINDEX(',', r.resourceGeoLocatin) WHEN 0 THEN LEN(r.resourceGeoLocatin) + 1 ELSE CHARINDEX(',', r.resourceGeoLocatin) + 1 END, 1000) AS lng, t.sheetInsertionDateTime ,DATEDIFF(minute, t.sheetInsertionDateTime, CONVERT(CHAR(24), CONVERT(DATETIME, GETDATE(), 103), 121)) as DeltaMinutes FROM tblSheet t inner join tblResource r on t.resourceID = r.ResourceID inner join tblResourceType rt on r.resourceTypeID = rt.resourceTypeID where rt.resourceTypeID = 1 and r.managedBy = " + Convert.ToInt32(Session["UserID"]) + " ORDER BY ROW_NUMBER() OVER(PARTITION BY r.resourceID  ORDER BY t.sheetInsertionDateTime DESC)";
                ...
                S2query += "inner join tblResource r on s.resourceID = r.resourceID ";
                S2query += "inner join tblResourceType rt on r.resourceTypeID = rt.resourceTypeID ";
                S2query += "where r.resourceTypeID = 1 and r.managedBy = " + Convert.ToInt32(Session["UserID"]) + " and sheetInsertionDateTime > DATEADD(MINUTE, -21, GETDATE())";
            ...
            using (SqlConnection conn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString))
            {
                try
                {
                    ...
                    ViewBag.InactivePonding = (22-(Convert.ToInt32(cmd2.ExecuteScalar()))).ToString();
        ...
        public ActionResult PondingReport()
        {
            ...
            using (SqlConnection conn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString))
            {
                try
                {
                    ...
                    foreach (DataRow drRes in dtRes.Rows)
                    {
                        ...
                        scriptString += "subtitles: [{text: \" All Ponding Locations between " + fromDt + " to " + toDt + " \" }],";
                        ...
                        foreach (DataRow drPar in dtPar.Rows)
                        {
                            ...
                            sdaVal.Fill(dtVal);
                            scriptString += "{ type: \"line\", name: \"" + drPar["resourceLocationName"].ToString() + "\", showInLegend: true,  markerSize: 0, xValueType: \"dateTime\", xValueFormatString: \"DD-MM-YYYY hh:mm:ss TT\", yValueFormatString: \"#,##0.##\", toolTipContent: \"{label}<br/>{name}, <strong>{y} inch</strong> at {x}\", ";
            ...
            var tablList = getPondingTableList(fromDt,toDt, "All");
            return View(tablList);
        }
        [HttpPost]
        public ActionResult PondingReport(FormCollection review)
        {
                ...
                using (SqlConnection conn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString))
                {
                    try
                    {
                        ...
                        foreach (DataRow drRes in dtRes.Rows)
                        {
                            ...
                            scriptString += "zoomEnabled: true, ";
                            scriptString += "title: {text: \"Ponding\" },";
                            scriptString += "subtitles: [{text: \" All Ponding Locations between " + FinalTimeFrom + " to " + FinalTimeTo + " \" }],";
                            scriptString += "axisY: {suffix: \" inch\" },";
                            //scriptString += "axisY: {includeZero: false, prefix: \"\", labelFormatter: function(e){if(e.value == NaN){return \"No Data\";}else{return e.value;}} },";
                            scriptString += "toolTip: { shared: false },";
                            scriptString += "legend: { cursor: \"pointer\", itemclick: toogleDataSeries},";
                            ...
                            foreach (DataRow drPar in dtPar.Rows)
                            {
                                ...
                                aquery += "FROM tblSheet e ";
                                aquery += "inner join tblResource r on e.resourceID = r.resourceID ";
                                aquery += "WHERE e.resourceID = " + Convert.ToInt32(drPar["resourceID"]) + " and e.parameterValue >= 0 and e.parameterID = " + Convert.ToInt32(drRes["parameterID"]) + " and e.sheetInsertionDateTime >= CONVERT(CHAR(24), CONVERT(DATETIME, '" + FinalTimeFrom + "', 103), 121) and e.sheetInsertionDateTime < CONVERT(CHAR(24), CONVERT(DATETIME, '" + FinalTimeTo + "', 103), 121)  ";
                                aquery += ") ";
                                aquery += "SELECT top 144000 parameterID, parameterValue, sheetInsertionDateTime FROM CTE WHERE RN < 144001 Order by sheetInsertionDateTime ASC";
                                string theQuery = aquery;
                                SqlDataAdapter sdaVal = new SqlDataAdapter(theQuery, conn);
                ...
                using (SqlConnection conn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString))
                {
                    try
                    {
                        ...
                        foreach (DataRow drRes in dtRes.Rows)
                        {
                            ...
                            scriptString += "zoomEnabled: true, ";
                            scriptString += "title: {text: \"Ponding\" },";
                            scriptString += "subtitles: [{text: \"Ponding at " + resource + " between " + FinalTimeFrom + " to " + FinalTimeTo + "\" }],";
                            scriptString += "axisY: {suffix: \" inch\" },";
                            //scriptString += "axisY: {includeZero: false, prefix: \"\", labelFormatter: function(e){if(e.value == NaN){return \"No Data\";}else{return e.value;}} },";
                            scriptString += "toolTip: { shared: false },";
                            scriptString += "legend: { cursor: \"pointer\", itemclick: toogleDataSeries},";
                            ...
                            foreach (DataRow drPar in dtPar.Rows)
                            {
                                ...
                                aquery += "FROM tblSheet e ";
                                aquery += "inner join tblResource r on e.resourceID = r.resourceID ";
                                aquery += "WHERE e.resourceID = " + Convert.ToInt32(drRes["resourceID"]) + " and e.parameterValue >= 0 and e.parameterID = " + Convert.ToInt32(drPar["parameterID"]) + " and e.sheetInsertionDateTime >= CONVERT(CHAR(24), CONVERT(DATETIME, '" + FinalTimeFrom + "', 103), 121) and e.sheetInsertionDateTime <= CONVERT(CHAR(24), CONVERT(DATETIME, '" + FinalTimeTo + "', 103), 121) ";
                                aquery += ") ";
                                aquery += "SELECT top 144000 parameterID, parameterValue, sheetInsertionDateTime FROM CTE WHERE RN < 144001 Order by sheetInsertionDateTime ASC";
                                string theQuery = aquery;
                                SqlDataAdapter sdaVal = new SqlDataAdapter(theQuery, conn);
                                DataTable dtVal = new DataTable();
                                sdaVal.Fill(dtVal);
                                scriptString += "{ type: \"line\", name: \"Ponding\", showInLegend: true,  markerSize: 0, xValueType: \"dateTime\", xValueFormatString: \"hh:mm:ss DD-MM-YYYY\", yValueFormatString: \"#,##0.##\", toolTipContent: \"{label}<br/>{name}, <strong>{y} inch</strong> at {x}\", ";
                                List<DataPoint> dataPoints = new List<DataPoint>();
                                DateTime dt = DateTime.Now;
                                foreach (DataRow drVal in dtVal.Rows)
                                {
            ...
            var tablList = getPondingTableList(FinalTimeFrom, FinalTimeTo, resource);
            return View(tablList);
        }
                    ...
                    try
                    {
                        ...
                        foreach (DataRow drRes in dtRes.Rows)
                        {
                            ...
                            foreach (DataRow drPar in dtPar.Rows)
                            {
                                ...
                                DataTable dtVal = new DataTable();
                                sdaVal.Fill(dtVal);
                                scriptString += "{ type: \"line\", name: \"Ponding\", showInLegend: true,  markerSize: 0, xValueType: \"dateTime\", xValueFormatString: \"DD-MM-YYYY hh:mm:ss TT\", yValueFormatString: \"#,##0.##\", toolTipContent: \"{label}<br/>{name}, <strong>{y} inch</strong> at {x}\", ";
                                List<DataPoint> dataPoints = new List<DataPoint>();
                                DateTime dt = DateTime.Now;
                                foreach (DataRow drVal in dtVal.Rows)
                                {
